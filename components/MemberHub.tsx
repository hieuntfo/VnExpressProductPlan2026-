...
const MemberHub: React.FC<MemberHubProps> = ({ projects, members, selectedMember, onSelectMember }) => {
  ...
  // Categorize members based on their job/position
  ...
  // Find projects for the selected member
  ...
  const renderSection = (title: string, stats: ReturnType<typeof getMemberStats>, colorClass: string, icon: React.ReactNode) => {
    if (!stats || stats.length === 0) return null; // Don't render empty sections
            ...
            <div key={member.name} onClick={() => onSelectMember(member)} className="bg-white/80 dark:bg-[#1e293b]/50 backdrop-blur-xl p-6 rounded-3xl shadow-lg border border-slate-200 dark:border-slate-700/50 hover:border-[#9f224e] hover:bg-white dark:hover:bg-[#1e293b]/70 hover:shadow-[0_0_20px_rgba(159,34,78,0.2)] transition-all group cursor-pointer active:scale-95 transform hover:-translate-y-1">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800 dark:text-white text-lg leading-tight group-hover:text-[#9f224e] transition-colors">{member.fullName}</h4>
                ...
                {member.active > 0 && (
                  <div className="flex flex-col items-center bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-900/60 dark:text-emerald-300 px-3 py-1 rounded-lg border dark:border-emerald-700 animate-pulse">
  ...
  const InfoRow = ({ label, value, href }: { label: string; value: string; href?: string }) => (
    <div className="flex items-center py-3 border-b border-slate-100 dark:border-slate-800">
      <span className="w-1/3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">{label}</span>
      {href ? (
        <a href={href} className="flex-1 font-bold text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 hover:underline">{value}</a>
      ) : (
        ...
        <div className="fixed inset-0 bg-slate-900/50 dark:bg-black/90 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-fade-in" onClick={() => onSelectMember(null)}>
          <div className="bg-white dark:bg-[#1e293b] w-full max-w-4xl h-[90vh] max-h-[700px] rounded-3xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700 animate-scale-in flex" onClick={e => e.stopPropagation()}>
            ...
            <div className="w-1/3 bg-slate-50 dark:bg-slate-900/50 p-8 flex flex-col items-center justify-center text-center border-r border-slate-200 dark:border-slate-800">
              <img src={selectedMember.avatar} alt={selectedMember.fullName} className="w-32 h-32 rounded-full shadow-2xl mb-6 border-4 border-white dark:border-[#1e293b]" />
              <h2 className="text-2xl font-black text-slate-900 dark:text-white leading-tight">{selectedMember.fullName}</h2>
              <p className="font-bold text-[#9f224e] dark:text-[#db2777] uppercase text-xs tracking-widest mt-2">{selectedMember.position}</p>
